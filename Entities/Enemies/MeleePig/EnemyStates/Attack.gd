...
@onready var after_attack_timer : Timer = $"../../AfterAttackTimer"
...
func startAfterTimer():
	after_attack_timer.start()

func enter(_msg := {}):
	if !after_attack_timer.is_connected("timeout", transition):
		after_attack_timer.timeout.connect(transition)
	if !attack_timer.is_stopped():
		state_machine.transition_to("Pursue")
		return
	attack_timer.start()
	after_attack_timer.paused = false
